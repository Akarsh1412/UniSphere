...
import { Menu, X, Home, PlusSquare, List, User, Megaphone, LogIn } from 'lucide-react';
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="relative px-4 py-2 text-sm font-medium transition-all duration-300 group text-gray-700 hover:text-blue-600 flex items-center space-x-2"
              >
                <item.icon size={18} />
                <span>{item.name}</span>
              </a>
            ...
            <button className="relative px-6 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-full overflow-hidden group transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/25 hover:scale-105 flex items-center space-x-2">
              <LogIn size={18} />
          ...
          <div className="pt-4 space-y-2">
            {navItems.map((item) => (
              <a
                ...
                className="flex items-center space-x-3 px-4 py-3 rounded-lg text-base font-medium transition-all duration-300 text-gray-700 hover:text-blue-600 hover:bg-gray-50"
              >
                <item.icon size={18} />
                <span>{item.name}</span>
              </a>
